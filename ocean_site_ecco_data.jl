...
function ecco_dataset(var, date; dir=config[:directories][:ecco_data_dir])
    ...
    filename = @match var begin
        ...
        "EXFempmr" => "OCEAN_AND_ICE_SURFACE_FW_FLUX_day_mean_$(datestr)_ECCO_V4r4_latlon_0p50deg.nc"
...
function _ecco_state(ds, var, idx_lat, idx_lon)
    return @match var begin
        "EXFtaue" || "EXFtaun" || "TFLUX" || "EXFempmr" || "MXLDEPTH" => ds[var][idx_lon, idx_lat, 1]
...
function gather_site_data(;
    ...
    ecco_vars = ["EXFtaue", "EXFtaun", "TFLUX", "EXFempmr", "MXLDEPTH", "EVEL", "NVEL", "THETA", "SALT", "RHOAnoma"]
